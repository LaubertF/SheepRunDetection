/**
 ...
 @author Ondrej Gallo<ondrej.gallo@stuba.sk>
*/
...
#define MPU9250_SLAVE_ADDRESS 0x69 ///< MPU9250 Default I2C slave address = 0x68 => AD0=LOW
...
class MPU9250 {
  public:
    TWILight sensor;
    MPU9250(uint8_t addr = MPU9250_SLAVE_ADDRESS);
    bool begin(void);
    ...
    bool checkDataReady();
    void readAccelXYZ2x8(uint8_t *ax, uint8_t *ay, uint8_t *az);
    TWIState readAccelXYZ2x8Volatile(volatile uint8_t *ax, volatile uint8_t *ay, volatile uint8_t *az);
    TWIState readAccGyMag2x8Volatile(volatile uint8_t *ax, volatile uint8_t *ay, volatile uint8_t *az,
    ...
    void readAccelXYZRaw(uint16_t *ax, uint16_t *ay, uint16_t *az);
    void readAccelXYZFloat(float * ax, float * ay, float * az);
